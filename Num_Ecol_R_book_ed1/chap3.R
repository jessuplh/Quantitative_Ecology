...
# on raw species data
# show the full matrix with diagonals (0.00)
round(vegdist(spe, diag=TRUE, upper=TRUE), 2)	# method="bray" (default)
# same, without the upper triangle
round(vegdist(spe, diag=TRUE), 2)
# same, without the diagonal
(spe.db <- round(vegdist(spe), 2))
head(spe.db)
# Percentage difference (Bray-Curtis) dissimilarity matrix
...
# Chord distance matrix
dev.new(title="Chord", width=10, height=5)
...
dev.new(title="S1 on species data", width=10, height=5)
...
dev.new(title="S1 on fictitious data", width=10, height=5)
...
range(dat2.S15)
dev.new(title="S15 on fictitious data - daisy", width=10, height=5)
coldiss(dat2.S15, diag=TRUE)

# Data matrix with the two orthogonal factors only
dat2partial.S15 <- daisy(dat2[,3:4], "gower")
dev.new(title="S15 on fictitious data, 2 factors - daisy", width=10, height=5)
...
range(dat2.S15.2)
dev.new(title="S15 on fictitious data - gowdis", width=10, height=5)
coldiss(dat2.S15.2, diag=TRUE)

# Data matrix with the two orthogonal factors only
dat2partial.S15.2 <- gowdis(dat2[,3:4])
dev.new(title="S15 on fictitious data, 2 factors - gowdis", width=10, height=5)
